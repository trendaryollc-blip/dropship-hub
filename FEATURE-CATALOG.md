...
| `/returns` · label generation           | Carrier return label                                                      | ⛔     | Deliberate HTTP **501**: "Return labels require a carrier integration (EasyPost or ShipStation)". No fabricated tracking numbers.                                                                         |
...
| #   | Item                              | Detail                                                                                                                   |
| --- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 1   | **Commit the work**               | ~343 files changed in the working tree, uncommitted (sprints + all audit waves). Not committed without owner's approval. |
| 2   | **Push & confirm CI green**       | `.github/workflows/ci.yml` must pass on GitHub after push.                                                               |
| 3   | **Obtain and configure API keys** | Tier 1 first (SerpAPI, RapidAPI Google Trends, Groq, CJ) — turns discovery/trends pages from honest-empty to live.       |
...
| 6   | **Carrier label integration**     | Build EasyPost/ShipStation integration to replace the honest `501` on return labels.                                     |
