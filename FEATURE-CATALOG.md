...
| Route                       | Feature                                                                                                              | Status | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/trends`                   | Trend analyzer: keyword signals, rising stars, lifecycle curve, keyword comparison, bulk analysis, PDF report export | ✅/🔑  | Live: Google Trends timeline + region map via `SERPAPI_KEYS` pool, Amazon BSR history via `KEEPA_API_KEYS`, Reddit (keyless public API). TikTok/Instagram/X request → honest "not connected / not wired up" note, never fake numbers. Missing keys: honest empty states and HTTP 400 "No live trend data for this keyword — connect a trends source". **No synthetic chart series** — real `interest_over_time` only, otherwise "Forecast unavailable" note. |
...
| ------------------ | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 1 (highest impact) | SerpAPI (`SERPAPI_KEYS`), CJ (`CJ_API_KEYS`), Keepa (`KEEPA_API_KEYS`)             | Product search, market intel, Google Trends signals, Amazon BSR, niche catalog, fulfillment |
| 2                  | Groq / OpenAI (`GROQ_API_KEYS`, `OPENAI_API_KEYS`)                                 | Chat assistant, due-diligence reports, listing text                                         |
| 3                  | Reddit (keyless — works today), TikTok / Instagram / X APIs (slots, not wired yet) | Social trend signals (Reddit live; others honestly "not connected")                         |
...
Minimum set to make discovery pages live: **SerpAPI + Keepa + Groq** (Reddit social signals need no key). Full table with signup URLs: [DATA_SOURCES.md](DATA_SOURCES.md). `.env.example` is the canonical env template.
