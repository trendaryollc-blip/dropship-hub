...
function HeatRing({ heat, size = 56 }: { heat: number; size?: number }) {
  ...
  const offset = circ - (heat / 100) * circ;
  const getColor = (v: number) => v >= 80 ? "#ef4444" : v >= 65 ? "#f97316" : v >= 50 ? "#f59e0b" : v >= 35 ? "#3b82f6" : "#64748b";
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full -rotate-90">
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="4" />
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={getColor(heat)} strokeWidth="4" strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={offset} className="transition-all duration-1000" />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="font-display text-sm font-bold text-foreground">{heat}</span>
      </div>
    </div>
  );
}
...
function MarketOverview({ categories }: { categories: HeatmapCategory[] }) {
  ...
  const avgHeat = Math.round(categories.reduce((s, c) => s + c.heat, 0) / categories.length);
  ...
  const r = 42, circ = Math.PI * r, offset = circ - (avgHeat / 100) * circ;
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="relative w-[100px] h-[55px] overflow-hidden">
            <svg viewBox="0 0 100 55" className="w-full h-full">
              ...
              <circle cx={5 + (avgHeat / 100) * 90} cy={50 - Math.sin((avgHeat / 100) * Math.PI) * 45} r="5" fill={getColor(avgHeat)} stroke="#0f0f17" strokeWidth="2" />
            </svg>
          </div>
          <div>
