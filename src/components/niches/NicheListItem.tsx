...
import { TrendingUp, TrendingDown, Minus, Flame, ArrowRight, DollarSign, GitCompare } from "lucide-react";
...
export default function NicheListItem({ niche, index, onSelect, onCompare, compareIds = [] }: NicheListItemProps) {
  ...
  return (
    <div
      ...
      className={`glass rounded-xl border p-3 sm:p-4 cursor-pointer transition-all duration-500 group ${isComparing ? "border-accent/40 shadow-[0_0_20px_rgba(var(--glow-color),0.15)]" : "border-border hover:border-accent/20"} ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
      style={{ transitionDelay: `${index * 40}ms` }}
    >
      <div className="flex items-center gap-3 sm:gap-4">
        <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-border shrink-0">
          <Image src={niche.image || "/placeholder.png"} alt={niche.name} width={64} height={64} unoptimized className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full border font-bold shrink-0 ${niche.riskLevel === "low" ? "text-emerald-400 bg-emerald-400/10 border-emerald-400/20" : niche.riskLevel === "medium" ? "text-amber-400 bg-amber-400/10 border-amber-400/20" : "text-red-400 bg-red-400/10 border-red-400/20"}`}>{niche.riskLevel} risk</span>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-all ${isComparing ? "text-accent bg-accent/10" : "text-muted-foreground hover:text-amber-400 hover:bg-amber-400/5"}`}
            title="Compare"
          >
