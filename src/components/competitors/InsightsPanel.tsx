...
export default function InsightsPanel({ insights }: { insights: string[] }) {
  ...
  return (
    <div ref={ref} className={`glass rounded-2xl p-4 sm:p-6 border border-border transition-all duration-700 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
      ...
      <div className="space-y-2.5">
        {insights.map((insight, i) => (
          <div
            key={i}
            onClick={() => handleCopy(insight, i)}
            className={`flex items-start gap-3 p-3 rounded-xl bg-surface/30 border border-border/30 hover:border-accent/15 transition-all duration-500 group cursor-pointer active:scale-[0.99] ${isInView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}
          ...
          >
            ...
            {copiedIdx === i ? (
              ...
              <Copy className="h-4 w-4 text-muted-foreground/30 group-hover:text-accent/50 transition-all shrink-0 mt-1" />
            )}
          </div>
        ))}
      </div>
