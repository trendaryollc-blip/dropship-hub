...
let msgIdCounter = 0;
function uid(): string { return `settings-chat-${Date.now()}-${++msgIdCounter}`; }

interface SettingsChatSidebarProps {
  providers: AIProvider[];
}

export default function SettingsChatSidebar({ providers }: SettingsChatSidebarProps) {
  ...
  const buildSystemContext = useCallback(() => {
    ...
    const providerList = configured.map((p) => `- ${p.name} (${p.envKey}) — used for: ${p.usedFor}`).join("\n");
    const inactiveCount = active.length - configured.length;
    return `You are a settings AI assistant for DropShip Hub. The user has ${configured.length} AI provider(s) configured out of ${providers.length} total.${providerList ? `\n\nConfigured providers:\n${providerList}` : "\n\nNo providers configured yet."}${inactiveCount > 0 ? `\n\n${inactiveCount} configured provider(s) are inactive.` : ""}\n\nHelp the user understand their settings, configure providers, and optimize their AI setup. You can explain what each provider does, recommend which to configure first, and troubleshoot issues.`;
  }, [providers]);
  ...
  const quickPrompts = [
    ...
    "Help me troubleshoot a connection",
  ];
  ...
  return (
    <>
      <button onClick={() => setOpen(!open)} className={`fixed bottom-6 right-6 z-30 p-4 rounded-2xl shadow-2xl transition-all ${open ? "bg-neutral-800 border border-neutral-700 text-neutral-300" : "bg-accent text-white hover:bg-accent-hover glow-accent"}`} title="Settings AI Assistant">
        ...
        <div className="fixed bottom-24 right-6 z-30 w-[380px] max-w-[calc(100vw-48px)] h-[520px] max-h-[calc(100vh-140px)] glass rounded-2xl border border-border shadow-2xl flex flex-col animate-in slide-in-from-bottom-4 fade-in duration-200">
          <div className="flex items-center justify-between p-4 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-semibold text-foreground">Settings AI</h3>
                <p className="text-[10px] text-muted-foreground">{providers.filter((p) => p.configured).length} provider{providers.filter((p) => p.configured).length !== 1 ? "s" : ""} configured</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && (
              <div className="space-y-3">
                <p className="text-xs text-muted-foreground text-center py-2">Ask anything about your settings</p>
                {quickPrompts.map((p) => (
