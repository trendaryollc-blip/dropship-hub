...
let msgIdCounter = 0;
function uid(): string {
  return `saved-chat-${Date.now()}-${++msgIdCounter}`;
}

export default function SavedChatSidebar() {
  ...
  const buildSystemContext = useCallback(() => {
    ...
    const productList = savedProducts
      .slice(0, 20)
      .map((p) => `- ${p.title} ($${p.price?.toFixed(2) ?? "N/A"}) from ${p.source}${p.rating ? ` ★${p.rating.toFixed(1)}` : ""}`)
      .join("\n");
    return `You are helping the user manage their ${savedProducts.length} saved products in DropShip Hub. Here are the saved products:\n${productList}\n\nYou can help analyze these products, suggest pricing, find suppliers, compare products, generate listings, and provide insights. Be concise and actionable.`;
  }, [savedProducts]);

  const quickPrompts = [
    "Which has the highest margin potential?",
    "Which products should I drop?",
    "Suggest pricing for all",
    "Find suppliers for top 3",
  ];
  ...
  return (
    <>
      <button
        ...
        title="AI Chat about saved products"
      >
        ...
        <div className="fixed bottom-24 right-6 z-30 w-[380px] max-w-[calc(100vw-48px)] h-[520px] max-h-[calc(100vh-140px)] glass rounded-2xl border border-border shadow-2xl flex flex-col animate-in slide-in-from-bottom-4 fade-in duration-200">
          <div className="flex items-center justify-between p-4 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-semibold text-foreground">Saved Products AI</h3>
                <p className="text-[10px] text-muted-foreground">{savedProducts.length} products loaded</p>
              </div>
            </div>
            ...
            {messages.length === 0 && (
              <div className="space-y-3">
                <p className="text-xs text-muted-foreground text-center py-2">
                  Ask anything about your saved products
                </p>
          ...
          <div className="p-3 border-t border-border">
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder="Ask about saved products..."
                rows={1}
