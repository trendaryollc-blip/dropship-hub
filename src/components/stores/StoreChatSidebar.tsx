...
export default function StoreChatSidebar({ connections, pushedProducts }: StoreChatSidebarProps) {
  ...
  const buildSystemContext = useCallback(() => {
    ...
    return `You are a store management AI for DropShip Hub. The user has ${connections.length} connected store(s):${storeList}${productSummary}\n\nHelp manage stores, push products, sync inventory, and optimize listings.`;
  }, [connections, pushedProducts]);
          ...
          <div className="flex items-center justify-between p-4 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">{connections.length} store{connections.length !== 1 ? "s" : ""} connected</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && (
              <div className="space-y-3">
                <p className="text-xs text-muted-foreground text-center py-2">Ask anything about your stores</p>
                {quickPrompts.map((p) => (
