...
import { formatMessage, generateChatMessageId } from "@/lib/chat-utils";
...
export default function StoreChat({ mode, connections, pushedProducts = [], orderCount = 0, totalRevenue = 0 }: StoreChatProps) {
  ...
  const isMultiStore = mode === "global" && pathname.startsWith("/multi-store");
  ...
  const handleSend = useCallback(async (text?: string) => {
    ...
    const userMsg: ChatMessage = { id: generateChatMessageId(baseId), role: "user", content, timestamp: new Date() };
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        if (res.status === 401) {
          setMessages((prev) => [...prev, { id: generateChatMessageId(baseId), role: "assistant", content: "Please sign in to use the store assistant.", timestamp: new Date() }]);
          return;
        }
        ...
        if (retry.response) setMessages((prev) => [...prev, { id: generateChatMessageId(baseId), role: "assistant", content: retry.response!, timestamp: new Date() }]);
        else setMessages((prev) => [...prev, { id: generateChatMessageId(baseId), role: "assistant", content: "Sorry, something went wrong. Please try again.", timestamp: new Date() }]);
        return;
      }
      const reader = res.body.getReader();
      const decoder = new TextDecoder();
      let full = "";
      const msgId = generateChatMessageId(baseId);
      ...
      setMessages((prev) => [...prev, { id: generateChatMessageId(baseId), role: "assistant", content: "Sorry, something went wrong.", timestamp: new Date() }]);
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-foreground">Store AI</h3>
