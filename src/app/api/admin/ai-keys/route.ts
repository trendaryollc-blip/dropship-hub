...
export interface AdminAiKeyEntry {
  id: string;
  key: string;
  label: string;
  priority: number;
...
export interface AdminAiProviderKeys {
  groq: AdminAiKeyEntry[];
  gemini: AdminAiKeyEntry[];
  openai: AdminAiKeyEntry[];
  deepseek: AdminAiKeyEntry[];
  mistral: AdminAiKeyEntry[];
  cohere: AdminAiKeyEntry[];
  together: AdminAiKeyEntry[];
  fireworks: AdminAiKeyEntry[];
  openrouter: AdminAiKeyEntry[];
  huggingface: AdminAiKeyEntry[];
  hpc: AdminAiKeyEntry[];
}

function generateKeyId(): string {
  return `akey_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
}
...
export const GET = requireOwner(async () => {
  try {
    ...
    const result: Record<string, { keys: Array<AdminAiKeyEntry & { masked: string }>; configured: boolean }> = {};
    for (const p of PROVIDERS) {
      const keys = ((data as Record<string, unknown>)[p] as AdminAiKeyEntry[]) || [];
      result[p] = {
...
export const POST = requireOwner(async (request: NextRequest) => {
  try {
    ...
    if (action === "add_key") {
      ...
      const newEntry: AdminAiKeyEntry = {
        id: generateKeyId(),
        key: key.trim(),
        label: label || "Primary",
        priority: 0,
      ...
      await db.runTransaction(async (tx) => {
        ...
        const existingKeys = (data[provider] as AdminAiKeyEntry[]) || [];
