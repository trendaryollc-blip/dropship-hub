...
describe("Banggood API Route", () => {
  ...
  it("GET returns platform info", async () => {
    ...
    const req = { url: "http://localhost/api/platforms/banggood" } as any;
    const res = await GET(req);
    const body = await res.json();
    expect(body.platform).toBe("Banggood");
  ...
  it("POST returns products on successful scrape", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      text: async () => '<div title="Banggood Product Widget"><img src="https://example.com/bg.jpg"></div><span>$25.00</span>',
    });
    const { POST } = await import("./route");
    const req = { json: async () => ({ query: "gadget" }) } as any;
    const res = await POST(req);
    const body = await res.json();
    expect(body.source).toBe("banggood");
