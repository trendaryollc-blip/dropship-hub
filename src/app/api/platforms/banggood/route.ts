...
export const POST = withAuth(async (request: NextRequest, uid: string) => {
  try {
    ...
    const { html, error: scraperError } = await scrapeUrl(`https://www.banggood.com/search/${encodeURIComponent(query)}.html`);
    if (!html) {
      return NextResponse.json({ data: { search_results: [] }, source: "banggood", query, scraperError: scraperError || "Failed to fetch page" });
    }

    const products = extractProducts(html, "Banggood");
    return NextResponse.json({ data: { search_results: products }, source: "banggood", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Banggood search failed", scraperError: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
});
