...
export const POST = withAuth(async (request: NextRequest, uid: string) => {
  try {
    ...
    const { html, error: scraperError } = await scrapeUrl(`https://www.dhgate.com/wholesale/search.html?searchkey=${encodeURIComponent(query)}`);
    if (!html) {
      return NextResponse.json({ data: { search_results: [] }, source: "dhgate", query, scraperError: scraperError || "Failed to fetch page" });
    }

    const products = extractProducts(html, "DHgate");
    return NextResponse.json({ data: { search_results: products }, source: "dhgate", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "DHgate search failed", scraperError: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
});

export const GET = withAuth(async (request: NextRequest, uid: string) => {
  return NextResponse.json({ platform: "DHgate", configured: !!(SCRAPER_API_KEY || ZENROWS_API_KEY) });
});
