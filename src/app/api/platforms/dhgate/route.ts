...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = await scrapeUrl(`https://www.dhgate.com/wholesale/search.html?searchkey=${encodeURIComponent(query)}`);
    const products = extractProducts(html, "DHgate");
    return NextResponse.json({ data: { search_results: products }, source: "dhgate", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "DHgate search failed" }, { status: 500 });
  }
}
