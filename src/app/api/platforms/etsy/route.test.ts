...
describe("Etsy API Route", () => {
  ...
  it("GET returns platform info", async () => {
    ...
    const req = { url: "http://localhost/api/platforms/etsy" } as any;
    const res = await GET(req);
    const body = await res.json();
    expect(body.platform).toBe("Etsy");
  ...
  it("POST returns products on successful scrape", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      text: async () => '<div title="Handmade Candle Holder"><img src="https://example.com/etsy.jpg"></div><span>$34.99</span>',
    });
    const { POST } = await import("./route");
    const req = { json: async () => ({ query: "candle holder" }) } as any;
    const res = await POST(req);
    const body = await res.json();
    expect(body.source).toBe("etsy");
