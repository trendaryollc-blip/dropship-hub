...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = await scrapeUrl(`https://www.etsy.com/search?q=${encodeURIComponent(query)}`);
    const products = extractProducts(html, "Etsy");
    return NextResponse.json({ data: { search_results: products }, source: "etsy", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Etsy search failed" }, { status: 500 });
  }
}
