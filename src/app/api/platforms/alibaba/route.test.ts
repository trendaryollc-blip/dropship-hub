...
describe("Alibaba API Route", () => {
  ...
  it("GET returns platform info", async () => {
    ...
    const req = { url: "http://localhost/api/platforms/alibaba" } as any;
  ...
  it("POST returns products on successful scrape", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      text: async () => '<div title="Test Product Widget"><img src="https://example.com/img.jpg"></div><span>$19.99</span>',
    });
    const { POST } = await import("./route");
    const req = { json: async () => ({ query: "test widget" }) } as any;
    const res = await POST(req);
    const body = await res.json();
    expect(body.source).toBe("alibaba");
    expect(body.data.search_results).toBeDefined();
    expect(Array.isArray(body.data.search_results)).toBe(true);
  });
});
