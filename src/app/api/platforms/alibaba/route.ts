...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = await scrapeUrl(`https://www.alibaba.com/trade/search?SearchText=${encodeURIComponent(query)}`);
    const products = extractProducts(html, "Alibaba");
    return NextResponse.json({ data: { search_results: products }, source: "alibaba", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Alibaba search failed" }, { status: 500 });
  }
}
