...
export const POST = withAuth(async (request: NextRequest, uid: string) => {
  try {
    ...
    const { html, error: scraperError } = await scrapeUrl(`https://www.alibaba.com/trade/search?SearchText=${encodeURIComponent(query)}`);
    if (!html) {
      return NextResponse.json({ data: { search_results: [] }, source: "alibaba", query, scraperError: scraperError || "Failed to fetch page" });
    }

    const products = extractProducts(html, "Alibaba");
    return NextResponse.json({ data: { search_results: products }, source: "alibaba", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Alibaba search failed", scraperError: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
});

export const GET = withAuth(async (request: NextRequest, uid: string) => {
  return NextResponse.json({ platform: "Alibaba", configured: !!(SCRAPER_API_KEY || ZENROWS_API_KEY) });
});
