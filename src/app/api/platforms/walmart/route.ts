...
export const POST = withAuth(async (request: NextRequest, uid: string) => {
  try {
    ...
    const { html, error: scraperError } = await scrapeUrl(`https://www.walmart.com/search?q=${encodeURIComponent(query)}`);
    if (!html) {
      return NextResponse.json({ data: { search_results: [] }, source: "walmart", query, scraperError: scraperError || "Failed to fetch page" });
    }

    const products = extractProducts(html, "Walmart");
    return NextResponse.json({ data: { search_results: products }, source: "walmart", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Walmart search failed", scraperError: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
});
