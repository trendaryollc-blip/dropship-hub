...
describe("Walmart API Route", () => {
  ...
  it("GET returns platform info", async () => {
    ...
    const req = { url: "http://localhost/api/platforms/walmart" } as any;
    const res = await GET(req);
    const body = await res.json();
    expect(body.platform).toBe("Walmart");
  ...
  it("POST returns products on successful scrape", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      text: async () => '<div title="Walmart Great Value TV"><img src="https://example.com/wm.jpg"></div><span>$299.99</span>',
    });
    const { POST } = await import("./route");
    const req = { json: async () => ({ query: "TV" }) } as any;
    const res = await POST(req);
    const body = await res.json();
    expect(body.source).toBe("walmart");
