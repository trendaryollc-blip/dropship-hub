...
export const POST = withAuth(async (request: NextRequest, uid: string) => {
  try {
    ...
    const { html, error: scraperError } = await scrapeUrl(`https://www.shein.com/pdsearch/${encodeURIComponent(query)}`);
    if (!html) {
      return NextResponse.json({ data: { search_results: [] }, source: "shein", query, scraperError: scraperError || "Failed to fetch page" });
    }

    const products = extractProducts(html, "SHEIN");
    return NextResponse.json({ data: { search_results: products }, source: "shein", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "SHEIN search failed", scraperError: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
});

export const GET = withAuth(async (request: NextRequest, uid: string) => {
  return NextResponse.json({ platform: "SHEIN", configured: !!(SCRAPER_API_KEY || ZENROWS_API_KEY) });
});
