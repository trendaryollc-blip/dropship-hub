...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `https://www.shein.com/pdsearch/${encodeURIComponent(query)}`;
    const html = await scrapeUrl(url);
    const products = extractProducts(html, "SHEIN");
    return NextResponse.json({ data: { search_results: products }, source: "shein", query });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "SHEIN search failed" }, { status: 500 });
  }
}
