...
export const GET = withAuth(async (req: NextRequest, uid: string) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch price rules", details: safeErrorMessage(error, "Unknown") }, { status: 500 });
  }
}, LIMITS.DEFAULT);
...
export const POST = withAuth(async (req: NextRequest, uid: string) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to create price rule", details: safeErrorMessage(error, "Unknown") }, { status: 500 });
  }
}, LIMITS.DEFAULT);
...
export const PATCH = withAuth(async (req: NextRequest, uid: string) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to update price rule", details: safeErrorMessage(error, "Unknown") }, { status: 500 });
  }
}, LIMITS.DEFAULT);
...
export const DELETE = withAuth(async (req: NextRequest, uid: string) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to delete price rule", details: safeErrorMessage(error, "Unknown") }, { status: 500 });
  }
}, LIMITS.DEFAULT);
