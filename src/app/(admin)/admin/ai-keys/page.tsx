...
export default function AdminAiKeysPage() {
  ...
  const fetchProviders = useCallback(async () => {
    ...
    try {
      ...
      const data = await safeFetch<{ providers?: Record<string, ProviderData> }>("/api/admin/ai-keys", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (data?.providers) setProviders(data.providers);
    } catch (err) {
      console.error("[AdminAiKeys] Failed to fetch:", err);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const handleAddKey = async (providerId: string) => {
    ...
    try {
      ...
      await safeFetch("/api/admin/ai-keys", {
        method: "POST",
  ...
  const startEditKey = (providerId: string, key: AdminAiKeyEntry) => {
  ...
  const handleTest = async (providerId: string, key: AdminAiKeyEntry) => {
    ...
    try {
      ...
      const data = await safeFetch<{ success: boolean; message: string }>("/api/platforms/admin/test", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...headers },
        body: JSON.stringify({ platformId: providerId, keyId: key.id, key: key.key, method: "ai_provider" }),
      });
  ...
  const handleDrop = async (providerId: string, targetKeyId: string) => {
    ...
    try {
      ...
      await fetchProviders();
    } catch (err) { console.error("[AdminAiKeys] Reorder failed:", err); }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">Loading AI provider keys...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8 max-w-6xl">
      <div>
        <h1 className="text-3xl font-display font-bold text-foreground mb-2 flex items-center gap-3">
          <Brain className="h-7 w-7 text-purple-400" /> AI Provider Keys
        </h1>
        <p className="text-sm text-muted-foreground">
          Manage shared AI provider API keys. These keys are used as fallback when users don&apos;t have their own keys configured.
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="glass rounded-xl p-4 border border-border">
          <div className="flex items-center gap-2 mb-1"><Key className="h-3.5 w-3.5 text-purple-400" /><span className="text-[10px] text-muted-foreground uppercase tracking-wider">Total Keys</span></div>
                ...
                <div className="border-t border-border p-5 space-y-4 bg-surface/30">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-semibold text-foreground flex items-center gap-2">
                      <Key className="h-4 w-4 text-purple-400" /> API Keys
                      ...
                      {data.keys.sort((a, b) => a.priority - b.priority).map((key) => (
                        <div key={key.id}
                          ...
                          className={`p-4 rounded-xl bg-surface/50 border transition-all ${
                            draggedKey?.keyId === key.id ? "opacity-50 border-purple-400/50"
                            : dragOverKey?.keyId === key.id ? "border-purple-400/50 bg-purple-400/5"
                            : "border-border"
                          }`}>
                            ...
                            <div className="space-y-3">
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Label</label>
                                  <input type="text" value={editKeyLabel} onChange={(e) => setEditKeyLabel(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-purple-400/50" /></div>
                                <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Request Limit</label>
                                  <input type="number" value={editKeyLimit} onChange={(e) => setEditKeyLimit(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-purple-400/50" /></div>
                                <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Reset Date</label>
                                  <input type="date" value={editKeyReset} onChange={(e) => setEditKeyReset(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-purple-400/50" /></div>
                              </div>
                              <div className="flex gap-2">
                                <button onClick={handleSaveEditKey} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-purple-500 text-white text-xs font-medium hover:bg-purple-600 transition-all">
                  ...
                  {showAddKeyFor === prov.id && (
                    <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/20 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Label</label>
                          <input type="text" value={addKeyLabel} onChange={(e) => setAddKeyLabel(e.target.value)} placeholder="e.g., Account 2" className="w-full mt-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-purple-400/50" /></div>
                        <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Request Limit</label>
                          <input type="number" value={addKeyLimit} onChange={(e) => setAddKeyLimit(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-purple-400/50" /></div>
                        <div><label className="text-[10px] text-muted-foreground uppercase tracking-wider">Resets On</label>
                          <input type="date" value={addKeyReset} onChange={(e) => setAddKeyReset(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground text-sm focus:outline-none focus:border-purple-400/50" /></div>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleAddKey(prov.id)} disabled={addingKey || !addKeyVal.trim()}
                          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-500 text-white text-xs font-medium hover:bg-purple-600 transition-all disabled:opacity-50">
      ...
      <ConfirmDialog open={!!confirmRemoveKey} title="Remove AI Provider Key" danger
