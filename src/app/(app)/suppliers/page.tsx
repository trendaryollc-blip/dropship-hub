...
function SuppliersContent() {
  ...
  // When product context exists, use the /api/suppliers/find endpoint
  ...
  const filtered = useMemo(() => {
    ...
    // When product context exists and no explicit sort, use relevance score
    ...
    <div className="max-w-7xl mx-auto space-y-5 md:space-y-6 pb-16 md:pb-24">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          {hasProductContext ? (
            <>Found <span className="text-accent font-medium">{suppliers.length}</span> suppliers matching this product</>
          ) : (
            <>Discover and analyze verified suppliers.{" "}<span className="text-accent font-medium">{suppliers.length}</span> supplier{suppliers.length !== 1 ? "s" : ""} with real data</>
          )}
        </p>
      </div>
      ...
      {/* Discovery Section (when no suppliers loaded) */}
